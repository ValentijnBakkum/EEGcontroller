...
for iter in range(max_iters):
        ...
        if iter % eval_interval == 0 or iter == max_iters - 1:
            avgloss = (np.sum(tlist)/(eval_interval-1))
            progress = (iter/max_iters) * 100
            print("validation loss : {}, progress : {}%, lr : {}".format(avgloss, int(progress), scheduler.get_last_lr()))
            if iter == 0:
                print(" ")
            else:
            ...
            lossv = torch.mean(lossvalue)
            #print(lossv)
            llist.append(lossv.data.cpu().numpy())
            optimizer.zero_grad(set_to_none=True)
            lossv.backward()
            optimizer.step()
            scheduler.step()
            with torch.no_grad(): #nograd because of memory leakage issue
