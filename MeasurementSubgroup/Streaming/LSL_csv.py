...
while True:
   ...
   try:
      ...
      logger.error(f"Invalid input received: {received}")
   ...
   if recieved_loop != "Stop":
      ...
      if Label1 is not None and not Label1.empty:
         Label1.to_csv('MeasurementSubgroup/Our_measurements/Measurement_prompt_labels/Label_' + now.strftime(
            "%Y-%j--%H-%M-%S") + '_1.csv', index=False)
      if Label2 is not None and not Label2.empty:
         Label2.to_csv('MeasurementSubgroup/Our_measurements/Measurement_prompt_labels/Label_' + now.strftime(
            "%Y-%j--%H-%M-%S") + '_2.csv', index=False)
      if Label3 is not None and not Label3.empty:
         Label3.to_csv('MeasurementSubgroup/Our_measurements/Measurement_prompt_labels/Label_' + now.strftime(
            "%Y-%j--%H-%M-%S") + '_3.csv', index=False)
      if Label4 is not None and not Label4.empty:
         Label4.to_csv('MeasurementSubgroup/Our_measurements/Measurement_prompt_labels/Label_' + now.strftime(
            "%Y-%j--%H-%M-%S") + '_4.csv', index=False)
   else:
      # Add column to the data_df called label
      data_df["Label"] = label # add new column to the Dataframe
